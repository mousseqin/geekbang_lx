...
func (n *node) childOf(path string) (*node, bool, bool) {
	...
	if n.children == nil {
		if n.starChild != nil && n.starChild.path == "*" {
			return n.starChild, false, true
		}
		if n.regChild != nil {
			if strings.Index(n.regChild.path, "(") > 0 && strings.Index(n.regChild.path, ")") > 0 {
				// 是正则
				sList := list.New()
				regExs := ""
				for _, s := range n.regChild.path {
					if s == '(' {
						sList.PushFront(s)
					}
					if s == ')' {
				...
				isMatch, _ := regexp.MatchString(regExs, path)
				if !isMatch {
					return n.regChild, false, false
				}
		...
		if strings.Index(n.regChild.path, "(") > 0 && strings.Index(n.regChild.path, ")") > 0 {
			...
			if !isMatch {
				if n.children[path] != nil {
					return n.children[path], false, true
				} else {
					return n.regChild, false, false
				}
			}
...
func (n *node) childOrCreate(path string) *node {
	...
	if path[0] == ':' {
		if strings.Index(path, "(") > 0 && strings.Index(path, ")") > 0 {
			...
			param := path[1:paramIndex]
			n.regChild = &node{path: path, paramName: param, typ: nodeTypeReg}
			return n.regChild
		} else {
			if n.paramChild != nil {
...
func (m *matchInfo) addValue(key string, value string) {
	if m.pathParams == nil {
		...
		if strings.Index(key, "(") > 0 && strings.Index(key, ")") > 0 {
			...
			paramIndex := strings.Index(key, "(")
			param := key[:paramIndex]
			m.pathParams = map[string]string{param: value}
			return
		} else {
